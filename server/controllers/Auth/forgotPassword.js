...
export const forgotPassword = async (req, res) => {
    try {
        ...
        console.log('Processing forgot password for email:', userEmail);
        ...
        try {
            await sendResetPasswordEmail(userEmail, resetCode);
            ...
            if (emailError.message && emailError.message.includes('chưa được cấu hình')) {
                // Trong development, có thể trả về mã để test (CHỈ DÙNG ĐỂ TEST!)
                if (process.env.NODE_ENV !== 'production') {
                    console.log('⚠️  EMAIL NOT CONFIGURED - Development mode:');
                    console.log('⚠️  Reset code for', userEmail, ':', resetCode);
                    console.log('⚠️  Please configure EMAIL_USER and EMAIL_PASS in .env file');
                    
                    // Giữ mã trong database để có thể test
                    return res.status(200).json({ 
                        success: true, 
                        message: 'Email chưa được cấu hình. Mã xác nhận đã được tạo (xem server logs).',
                        // CHỈ TRONG DEVELOPMENT - XÓA TRONG PRODUCTION!
                        resetCode: process.env.NODE_ENV !== 'production' ? resetCode : undefined,
                        warning: 'Email service chưa được cấu hình. Vui lòng xem HUONG_DAN_CAU_HINH_EMAIL.md'
                    });
                }
            }
            
            // Nếu gửi email thất bại, xóa mã reset đã lưu
